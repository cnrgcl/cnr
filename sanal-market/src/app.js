...
(function () {
  ...
  // Ürünün içi — panelde "aç" denince ortaya çıkan dolgu.
  // r: dolgu rengi, t: sivi | akiskan | granul | kati | etli, d: doluluk oranı
  var ICLER = {
    bal:{r:'#E9A70F',t:'akiskan',d:0.86},      recel:{r:'#8E1B2B',t:'akiskan',d:0.84},
    krema:{r:'#4A2A14',t:'akiskan',d:0.86},    yogurt:{r:'#F4F7F6',t:'akiskan',d:0.90},
    gevrek:{r:'#D9A23C',t:'granul',d:0.72},    cay:{r:'#3A2A18',t:'granul',d:0.78},
    kahve:{r:'#5A3418',t:'granul',d:0.80},     cikolata:{r:'#3A2114',t:'kati',d:0.92},
    peynir:{r:'#F4F6F3',t:'kati',d:0.88},      tereyagi:{r:'#F0D46A',t:'kati',d:0.90},
    pecete:{r:'#FBFCFB',t:'kati',d:0.90},      pirinc:{r:'#EDE6D6',t:'granul',d:0.80},
    makarna:{r:'#D9A441',t:'granul',d:0.80},   mercimek:{r:'#D2532A',t:'granul',d:0.80},
    un:{r:'#F0EADC',t:'granul',d:0.84},        cips:{r:'#E8C14A',t:'granul',d:0.60},
    findik:{r:'#8A5A2A',t:'granul',d:0.76},    seker:{r:'#E8709F',t:'granul',d:0.76},
    kuruyemis:{r:'#B08A46',t:'granul',d:0.76}, havlu:{r:'#FBFCFB',t:'kati',d:0.90},
    tuvalet:{r:'#FBFCFB',t:'kati',d:0.90},     nohut:{r:'#D9C48A',t:'granul',d:0.82},
    ton:{r:'#C97A4A',t:'granul',d:0.80},       su:{r:'#BFE6F5',t:'sivi',d:0.90},
    gazoz:{r:'#E8D25A',t:'sivi',d:0.88},       zeytin:{r:'#9AA82A',t:'sivi',d:0.88},
    ayran:{r:'#F4F6F5',t:'sivi',d:0.88},       bulasik:{r:'#5FCB92',t:'sivi',d:0.86},
    camasir:{r:'#CFE4F2',t:'sivi',d:0.86},     sabun:{r:'#CBD86A',t:'sivi',d:0.86},
    sut:{r:'#F6F8F7',t:'sivi',d:0.90},         visne:{r:'#7E1428',t:'sivi',d:0.90},
    domates:{r:'#E06A56',t:'etli',d:1},        elma:{r:'#F2E6BC',t:'etli',d:1},
    limon:{r:'#F6E28C',t:'etli',d:1},          salatalik:{r:'#E2EFCF',t:'etli',d:1}
  };
  function icBilgi(p) { return ICLER[p.id] || { r: tonla(p.renk, 0.2), t: 'kati', d: 0.86 }; }
  ...
  function govdeKutu(p) {
    ...
    var parcalar = [
      { geo: kutuGeo, mat: yan, rol: 'kabuk' },
      { geo: levha(en * 0.995, boy * 0.995, derin / 2 + 0.0012, false), mat: em, rol: 'etiket' },
      { geo: levha(en * 0.995, boy * 0.995, derin / 2 + 0.0012, true), mat: em, rol: 'etiket' }
    ];

    var ib = icBilgi(p);
    var icBoy = boy * ib.d - 0.006;
    var icGeo = new T.BoxGeometry(en * 0.9, icBoy, derin * 0.84);
    icGeo.translate(0, icBoy / 2 + 0.004, 0);
    parcalar.push({ geo: icGeo, mat: govdeMat(ib.r, ib.t === 'sivi'), rol: 'ic', panelde: true });

    if (p.tip === 'tetra') {
      var kap = new T.BoxGeometry(en * 0.44, 0.014, derin * 0.44);
      kap.translate(0, boy + 0.007, 0);
      parcalar.push({ geo: kap, mat: govdeMat('#D8DCD8', true), rol: 'kapak' });
    } else {
      var kapakGeo = new T.BoxGeometry(en, 0.008, derin);
      kapakGeo.translate(0, boy - 0.004, 0);
      parcalar.push({ geo: kapakGeo, mat: govdeMat(tonla(p.renk, -0.18), false), rol: 'kapak', panelde: true });
    }
    return { parcalar: parcalar, en: en, boy: boy, derin: derin, icYuzey: boy * ib.d };
  }
  ...
  function govdePoset(p) {
    ...
    var ib = icBilgi(p);
    var icBoy = boy * ib.d - 0.012;
    var icGeo = new T.BoxGeometry(en * 0.86, icBoy, derin * 1.4);
    icGeo.translate(0, icBoy / 2 + 0.006, 0);

    return {
      parcalar: [
        { geo: geo, mat: yan, rol: 'kabuk' },
        { geo: kirma, mat: yan, rol: 'kapak' },
        { geo: sisikLevha(false), mat: em, rol: 'etiket' },
        { geo: sisikLevha(true), mat: em, rol: 'etiket' },
        { geo: icGeo, mat: govdeMat(ib.r, false), rol: 'ic', panelde: true }
      ],
      en: en, boy: boy, derin: derin * 1.85, icYuzey: boy * ib.d
    };
  }
  ...
  function govdeSise(p) {
    ...
    // İçindeki sıvı — şişe yarı saydam olduğu için rafta da görünür
    var ib = icBilgi(p);
    var seviye = boy * 0.66 * ib.d + 0.02;
    var siviGeo = new T.LatheGeometry([
      new T.Vector2(0.001, 0.008),
      new T.Vector2(r * 0.9, 0.022),
      new T.Vector2(r * 0.945, boy * 0.55),
      new T.Vector2(r * 0.9, seviye),
      new T.Vector2(0.001, seviye)
    ], 22);

    return {
      parcalar: [
        { geo: siviGeo, mat: govdeMat(ib.r, true), rol: 'ic' },
        { geo: geo, mat: siseMat, rol: 'kabuk' },
        { geo: kap, mat: govdeMat(kapakRenk, true), rol: 'kapak' },
        { geo: etGeo, mat: etiketMat(p, 'sarma', etH / (Math.PI * r), true), rol: 'etiket' }
      ],
      en: r * 2, boy: boy + 0.03, derin: r * 2, icYuzey: seviye
    };
  }
  ...
  function govdeTeneke(p) {
    ...
    var ib = icBilgi(p);
    var icBoy = boy * ib.d - 0.008;
    var icGeo = new T.CylinderGeometry(r * 0.94, r * 0.94, icBoy, 22);
    icGeo.translate(0, icBoy / 2 + 0.004, 0);

    return {
      parcalar: [
        { geo: govde, mat: etiketMat(p, 'sarma', boy / (Math.PI * r), true), rol: 'etiket' },
        { geo: ustGeo, mat: metalMat('#C9CED2'), rol: 'kabuk' },
        { geo: altGeo, mat: metalMat('#C9CED2'), rol: 'kabuk' },
        { geo: kapak, mat: metalMat('#B4B9BD'), rol: 'kapak' },
        { geo: icGeo, mat: govdeMat(ib.r, false), rol: 'ic', panelde: true }
      ],
      en: r * 2.1, boy: boy, derin: r * 2.1, icYuzey: boy * ib.d
    };
  }
  ...
  function govdeKavanoz(p) {
    ...
    var icGovde = new T.CylinderGeometry(r * 0.92, r * 0.88, icBoy, 22);
    icGovde.translate(0, icBoy / 2 + 0.006, 0);
    ...
    return {
      parcalar: [
        { geo: icGovde, mat: govdeMat(ib.r, ib.t === 'akiskan'), rol: 'ic' },
        { geo: cam, mat: camMat('#E8F0EE', p.id === 'yogurt' ? 0.9 : 0.42), rol: 'kabuk' },
        { geo: kapak, mat: govdeMat(p.id === 'yogurt' ? '#2C6FA8' : tonla(p.renk, -0.22), true), rol: 'kapak' },
        { geo: etGeo, mat: etiketMat(p, 'sarma', etH / (Math.PI * r), true), rol: 'etiket' }
      ],
      en: r * 2.1, boy: boy + 0.03, derin: r * 2.1, icYuzey: icBoy
    };
  }
  ...
  function govdeMeyve(p) {
    ...
    if (p.id === 'salatalik') {
      ...
      var icG = new T.CapsuleGeometry(0.020, 0.098, 3, 10);
      icG.rotateZ(Math.PI / 2);
      icG.translate(0, 0.026, 0);
      return {
        parcalar: [
          { geo: icG, mat: govdeMat(ib.r, false), rol: 'ic', panelde: true },
          { geo: g, mat: govdeMat(p.renk, true), rol: 'kabuk' }
        ],
        en: 0.16, boy: 0.055, derin: 0.055, icYuzey: 0.05
      };
    }
    var r = 0.043;
    ...
    var etGeo = new T.SphereGeometry(r * 0.9, 12, 9);
    etGeo.scale(1, yassilik, 1);
    etGeo.translate(0, r * yassilik, 0);
    var parcalar = [
      { geo: etGeo, mat: govdeMat(ib.r, false), rol: 'ic', panelde: true },
      { geo: kure, mat: govdeMat(p.renk, true), rol: 'kabuk' }
    ];
    if (p.id !== 'limon') {
      var sap = new T.CylinderGeometry(0.0035, 0.005, 0.026, 5);
      sap.translate(0, r * yassilik * 2 + 0.008, 0);
      parcalar.push({ geo: sap, mat: govdeMat('#4A5A20', false), rol: 'kapak' });
      var cek = new T.SphereGeometry(r * 0.1, 6, 5);
      cek.scale(0.7, 1.4, 1);
      for (var ci = 0; ci < 5; ci++) {
        var acili = cek.clone();
        acili.translate(Math.cos(ci * 1.257) * r * 0.36, r * yassilik * 0.92, Math.sin(ci * 1.257) * r * 0.36);
        parcalar.push({ geo: acili, mat: govdeMat('#4A3A1E', false), rol: 'ic', panelde: true });
      }
    }
    return { parcalar: parcalar, en: r * 2.1, boy: r * yassilik * 2.2, derin: r * 2.1, icYuzey: r * yassilik * 1.8 };
  }
  ...
  var granulGeolar = null;
  function granulSekilleri() {
    if (!granulGeolar) {
      granulGeolar = [
        new T.TetrahedronGeometry(0.5),
        new T.BoxGeometry(0.95, 0.45, 0.62),
        new T.SphereGeometry(0.5, 6, 5),
        new T.CylinderGeometry(0.28, 0.28, 0.9, 6)
      ];
    }
    return granulGeolar;
  }

  // Dolgunun üstüne tek tek taneler serpiştir — "içi göründü" hissi buradan gelir
  function granulSerp(grup, p, g, ib) {
    var sekiller = granulSekilleri();
    var tonlar = [govdeMat(ib.r, false), govdeMat(tonla(ib.r, -0.13), false), govdeMat(tonla(ib.r, 0.12), false)];
    var boyut = Math.min(g.en, g.derin) * 0.2;
    var yuzey = g.icYuzey || g.boy * 0.8;
    var eskiTohum = tohum;
    tohum = 4021 + p.id.length * 977;
    for (var i = 0; i < 44; i++) {
      var m = new T.Mesh(sekiller[i % sekiller.length], tonlar[i % 3]);
      var ol = boyut * rastArasi(0.7, 1.25);
      m.scale.setScalar(ol);
      m.position.set(
        rastArasi(-g.en * 0.36, g.en * 0.36),
        yuzey - rastArasi(0, boyut * 1.1),
        rastArasi(-g.derin * 0.28, g.derin * 0.28));
      m.rotation.set(rastArasi(0, 6.28), rastArasi(0, 6.28), rastArasi(0, 6.28));
      m.userData.rol = 'ic';
      grup.add(m);
    }
    tohum = eskiTohum;
  ...
  function urunGrubu(p) {
    ...
    g.parcalar.forEach(function (par) {
      var mat = par.mat.clone();
      var m = new T.Mesh(par.geo, mat);
      m.userData.rol = par.rol || 'kabuk';
      m.userData.temelSaydam = mat.opacity;
      m.userData.temelYuz = mat.side;
      grup.add(m);
    });
    if (ib.t === 'granul') granulSerp(grup, p, g, ib);
  ...
  function urunleriYerlestir() {
    ...
    URUNLER.forEach(function (p) {
      ...
      govde(p).parcalar.forEach(function (par) {
        if (par.panelde) return;   // yalnızca panelde görünen dolgu parçaları
  ...
  var pUrun = null, icAcik = false, acikT = 0, pBoyOlcekli = 0;
  ...
  function panelKur() {
    ...
    document.getElementById('acDugme').addEventListener('click', function () {
      icAcik = !icAcik;
      this.setAttribute('aria-pressed', icAcik ? 'true' : 'false');
      this.querySelector('span').textContent = icAcik ? 'Kapat' : 'İçini aç';
      document.getElementById('rozetYazi').textContent = icAcik ? 'İçi açık' : 'Canlı 3B';
    });
  }
  ...
  function urunAc(p) {
    ...
    pUrun = grup;
    pBoyOlcekli = o.boy;
    icAcik = false; acikT = 0;
    acilmaUygula();
    var ac = document.getElementById('acDugme');
    ac.setAttribute('aria-pressed', 'false');
    ac.querySelector('span').textContent = 'İçini aç';
    document.getElementById('rozetYazi').textContent = 'Canlı 3B';

    pSapma = 0.6; pEgim = 0.16;
    panelBoyutla();
  }

  // Kapağı kaldır, kabuğu şeffaflaştır: ürünün içi ortaya çıksın
  function acilmaUygula() {
    if (!pUrun) return;
    var t = acikT * acikT * (3 - 2 * acikT);
    pUrun.children.forEach(function (m) {
      var rol = m.userData.rol;
      if (rol === 'kapak') {
        m.position.y = t * pBoyOlcekli * 0.46;
        m.position.x = t * pBoyOlcekli * 0.34;
        m.rotation.z = -t * 0.7;
      } else if (rol === 'kabuk' || rol === 'etiket') {
        var temel = m.userData.temelSaydam;
        var hedef = rol === 'etiket' ? 0.16 : 0.26;
        var o = temel + (Math.min(temel, hedef) - temel) * t;
        m.material.opacity = o;
        m.material.transparent = o < 0.999;
        m.material.depthWrite = o > 0.72;
        // Açıkken kabuk cam bir kap gibi okunsun: arka yüzler de görünsün
        m.material.side = t > 0.02 ? T.DoubleSide : m.userData.temelYuz;
      }
    });
  }
  ...
  function dongu(zaman) {
    ...
    if (panelAcikMi) {
      var hedefAcik = icAcik ? 1 : 0;
      if (Math.abs(acikT - hedefAcik) > 0.0015) {
        acikT += (hedefAcik - acikT) * Math.min(1, dt * 5.5);
        acilmaUygula();
      }
